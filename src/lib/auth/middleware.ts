 ...
 * (see `verify.server.ts`). Auth disabled does not fall back to a shared dev
 * user on the server. A production deploy with `VITE_AUTH_ENABLED=false` must
 * not attribute every booking to one id. On the auth-on path, use this on
...
export const authMiddleware = createMiddleware({ type: "function" })
  ...
  .server(async ({ next, context }) => {
    ...
    const { UnauthorizedError, getSessionUser } = await import("./verify.server");
